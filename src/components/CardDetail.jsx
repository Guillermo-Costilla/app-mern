...
export default function CardDetail({ city, image, country, description, itineraries, activities }) {
    ...
    return (
        <div className='flex mt-10 ml-5 sm:max-lg:flex-col'>
            <div className='w-1/12 relative sm:max-lg:mb-5'>
                <div className='flex flex-col justify-left sm:max-lg:flex-row'>
                    ...
                    <Link key={Links[1].id} to={Links[1].to}>
                        <button className='mt-4 sm:max-lg:mt-0 sm:max-lg:ml-4 text-white p-2 text-xl font-bold transition ease-in-out delay-130 hover:-translate-y-1 hover:scale-110 hover:drop-shadow-xl duration-200 boton'>{Links[1].title}</button>
                    </Link>
                </div>
            </div>
            <div className='flex flex-col sm:w-full md:w-5/6 gap-4 rounded-lg'>
